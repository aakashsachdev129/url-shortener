...
export class UrlRepository implements IUrlRepository {
  ...
  async get(shortUrl: string, ip: string): Promise<string> {
    try {
      ...
      if (result && result.longUrl) {
        ...
        if (
          requestLimit > 0 &&
          sumVisitCount.length > 0 &&
          sumVisitCount[0].totalRequestCount >= requestLimit
        ) {
          // Throw an exception if the request limit has reached
          throw new HttpException(
            'Access to this URL cannot be granted as the visit request limit has been reached!',
  ...
  async setAlias(url: Url): Promise<string> {
    try {
      const exists = await this.urlModel
      ...
      const aliasExists = await this.urlModel
  ...
  async deleteUrl(shortUrl: string): Promise<string> {
    try {
      ...
      await this.urlModel
        .findOneAndUpdate(
          { shortUrl: shortUrl },
          { isDeleted: true, deletedOn: new Date() },
          { new: true },
        )
        .exec();
