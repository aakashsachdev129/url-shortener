...
export class UrlController {
  ...
  async getStatistics(@Res() res: Response) {
    try {
      const allStats = await this.urlService.getStatistics();
  ...
  async getAndRedirect(
    @Param('urlCode') urlCode: string,
    @Ip() ip: string,
    @Res() res: Response,
  ) {
    try {
      const originalUrl = await this.urlService.getLongUrl(urlCode, ip);
  ...
  async setAlias(@Body() setAliasDto: SetAliasDto, @Res() res: Response) {
    try {
      ...
      if (
        !isURL(setAliasDto.shortUrl) &&
        !setAliasDto.shortUrl.startsWith(urlShortenerDomain)
      ) {
        throw new HttpException(
          'shortURL must be an auto generated short URL or a valid long URL',
          HttpStatus.BAD_REQUEST,
        );
      }

      const aliasUrl = await this.urlService.setAlias(
        setAliasDto.shortUrl,
        setAliasDto.alias,
      );
  ...
  async setRequestLimit(
    ...
    try {
      const response = await this.urlService.setRequestLimit(
        setRequestLimitDto.shortUrl,
        setRequestLimitDto.requestLimit,
      );
  ...
  async deleteUrl(@Body() deleteUrlDto: DeleteUrlDto, @Res() res: Response) {
    try {
      const response = await this.urlService.deleteUrl(deleteUrlDto.shortUrl);
      return res.status(HttpStatus.OK).json(response);
    } catch (error) {
      throw new HttpException(error.response, error.status);
    }
